...
fun EditorScreen(
    ...
    var dateText by rememberSaveable(initialTime?.date?.toString()) { mutableStateOf(initialTime?.date?.toString() ?: "") }
    var timeText by rememberSaveable(initialTime?.formattedTime()) { mutableStateOf(initialTime?.formattedTime() ?: "") }
    var zoneIdText by rememberSaveable(initialTime?.zoneId?.id ?: ZoneId.systemDefault().id) {
        ...
        Column(
            ...
            OutlinedTextField(
                ...
                keyboardActions = KeyboardActions(
                    onDone = {
                        if (title.isNotBlank()) {
                            val timePoint = buildTimePoint(dateText, timeText, zoneIdText, timeFormatter)
                    ...
                    Priority.values().forEach { priority ->
                        FilterChip(
                            ...
                            colors = FilterChipDefaults.filterChipColors(
                                selectedContainerColor = priorityColor(priority).copy(alpha = 0.2f),
            ...
            Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                val zoneDisplay = runCatching { ZoneId.of(zoneIdText) }.getOrDefault(ZoneId.systemDefault())
    ...
    if (schedulePickerOpen) {
        ScheduleBottomSheet(
            ...
            initialTime = timeText.takeIf { it.isNotBlank() }?.let { LocalTime.parse(it, timeFormatter) },
            initialZoneId = runCatching { ZoneId.of(zoneIdText) }.getOrElse { ZoneId.systemDefault() },
...
private fun ScheduleBottomSheet(
    ...
    var selectedZoneId by rememberSaveable(initialZoneId.id) { mutableStateOf(initialZoneId.id) }
    val allZones = remember { ZoneId.getAvailableZoneIds().sorted() }
    val filteredZones = remember(zoneQuery, allZones) {
        val query = zoneQuery.trim()
    ...
    ModalBottomSheet(
        ...
        Column(
            ...
            if (includeTime) {
                val selectedZone = runCatching { ZoneId.of(selectedZoneId) }.getOrElse { systemZone }
                val currentTime = LocalTime.of(timeState.hour, timeState.minute)
                val timeDisplay = currentTime.format(DateTimeFormatter.ofPattern("h:mm a", Locale.getDefault()))
                Text(
            ...
            Row(
                ...
                Button(
                    onClick = {
                        ...
                        val date = Instant.ofEpochMilli(selectedMillis).atZone(systemZone).toLocalDate()
                        val zone = runCatching { ZoneId.of(selectedZoneId) }.getOrElse { systemZone }
        ...
        Column(
            ...
            Column(
                ...
                filteredZones.forEach { zoneId ->
                    ...
                    val shorthand = display?.getDisplayName(TextStyle.SHORT, Locale.getDefault())
                    val label = if (shorthand.isNullOrBlank()) zoneId else "$zoneId ($shorthand)"
                    TextButton(
                        onClick = {
                            onZoneSelected(zoneId)
                            scope.launch {
                        ...
                        Text(
                            text = label,
                            style = if (zoneId == selectedZoneId) MaterialTheme.typography.bodyLarge
                ...
                if (filteredZones.isEmpty()) {
                    Text("No time zones found", style = MaterialTheme.typography.bodySmall)
                }
            }
...
private fun buildTimePoint(
    ...
    val time = timeText.takeIf { it.isNotBlank() }?.let { LocalTime.parse(it, timeFormatter) }
    val zone = runCatching { ZoneId.of(zoneIdText) }.getOrElse { ZoneId.systemDefault() }
